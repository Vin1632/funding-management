...
const ManagersHome = ({ email }) => {
  ...
  const fetchBudgetInformation = async () => {
    try {
      ...
      // Update state with fetched budget information
      setEducationBudget(data.educationBudget);
      setEventsBudget(data.eventsBudget);
      setBusinessBudget(data.businessBudget);
    } catch (error) {
